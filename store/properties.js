...
export const getters = {
  ...
  properties_by_destinations(state) {
    return state.properties_by_destinations;
  },
...
export const mutations = {
  ...
  SET_PROPERTIES_BY_DESTINATIONS(state, property) {
    state.properties_by_destinations = property;
  }
};
...
export const actions = {
  ...
  async getPropertiesByDestinations({ commit }, type, page) {
    try {
      let res = await this.$axios.$get(
        `properties-by-destinations/${type}?page=${page}`
      );
      commit("SET_PROPERTIES_BY_DESTINATIONS", res.data);
      commit("SET_PAGINATION", res);
    } catch (error) {}
  },

  async getPropertyByDestination({ commit, getters }, slug) {
    let property = getters.properties_by_destinations.find(
      property => property.slug === slug
    ); // Boa!!!!!!
    if (property) {
      commit("SET_PROPERTY", property);
